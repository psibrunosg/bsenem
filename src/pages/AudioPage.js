...
export class AudioPage {
  ...
  async render() {
    ...
    this.element.innerHTML = '<header class="page-header"><h1>Áudios</h1><p>Ouça materiais da sua biblioteca local.</p></header><div class="lesson-player-container"></div><p class="local-media-state" role="status"></p>';
    if (!pending) return this.empty('Selecione um áudio na biblioteca local.');
    if (!pending.lesson) return this.empty(pending.error || 'Arquivo local indisponível. Atualize a biblioteca e tente novamente.');
    try {
      this.player = new LessonPlayer({ lesson: pending.lesson, initialMode: 'audio', library: this.library });
